...
public class ESUtil {
    ...
    public static boolean checkIndexExist(String index) {
        ...
        if (inExistsResponse.isExists()) {
            LOGGER.info("CheckIndex, Index {} is exist!", index);
        } else {
            LOGGER.info("CheckIndex, Index {} is not exist!", index);
        }
    ...
    public static boolean createIndex(String index) {
        if (!checkIndexExist(index)) {
            LOGGER.info("CreateIndex, Index is not exits!");
        }
    ...
    /**
     * TODO:通过ID 更新数据，不建议使用更新数据。最佳实践为删除后重新创建
     *
     * @param jsonObject 要增加的数据
     * @param index      索引，类似数据库
     * @param type       类型，类似表
